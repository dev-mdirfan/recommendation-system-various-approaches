...
from recommend4 import *
app = Flask(__name__)

@app.route('/')
def index4():
    data1 = pd.read_csv('static/data/recommendData4.csv')
    # product_name = data['product_name']
    data2 = df.original_data
    # Merging data together
    data = pd.merge(data1, data2, on='product_name', how='inner')
    data.to_json('./static/data/recommendData4.json')
    # jsonify(data)
    ...
    return render_template('index4.html', exp = {'data':data, 'tables':tables, 'query':''})
...
def predict(query):
    if request.method == 'GET':
        ...
        data = data.to_dict('records')
        data = [tuple(x.values()) for x in data]
        return render_template('index4.html', exp = {'data':data, 'tables':tables, 'query':query})
